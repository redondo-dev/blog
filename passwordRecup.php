...
if (isset($_POST['recup_submit'], $_POST['recup_mail'])) {
   if (!empty($_POST['recup_mail'])) {
      ...
      if (filter_var($recup_mail, FILTER_VALIDATE_EMAIL)) {
         $mailexist = $pdo->prepare('SELECT id,pseudo FROM users WHERE mail = ?');
         ...
         if ($mailexist_count == 1) {
            ...
            if ($mail_recup_exist == 1) {
               $recup_insert = $pdo->prepare('UPDATE recuperation SET code = ? WHERE mail = ?');
         ...
         ';
            ...
            header("Location:http:localhost/recuperation.php?section=code");
         } else {
...
if (isset($_POST['verif_submit'], $_POST['verif_code'])) {
   if (!empty($_POST['verif_code'])) {
      ...
      if ($verif_req == 1) {
         ...
         header('Location:http:localhost/recuperation.php?section=changemdp');
      } else {
         $error = "Code invalide";
      }
   } else {
...
if (isset($_POST['change_submit'])) {
   if (isset($_POST['change_mdp'], $_POST['change_mdpc'])) {
      $verif_confirme = $pdo->prepare('SELECT confirme FROM recuperation WHERE mail = ?');
      ...
      if ($verif_confirme == 1) {
         ...
         if (!empty($mdp) and !empty($mdpc)) {
            if ($mdp == $mdpc) {
               $mdp = sha1($mdp);
               $ins_mdp = $pdo->prepare('UPDATE users SET motdepasse = ? WHERE mail = ?');
               $ins_mdp->execute(array($mdp, $_SESSION['recup_mail']));
               $del_req = $pdo->prepare('DELETE FROM recuperation WHERE mail = ?');
